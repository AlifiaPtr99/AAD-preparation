    ...
    override fun onBindViewHolder(holder: TaskViewHolder, position: Int) {
        ...
        when {
            ...
            task.isCompleted -> {
                ...
                holder.title.state = TaskTitleView.DONE
            }
            task.dueDateMillis < System.currentTimeMillis() -> {
                //OVERDUE
                holder.checkbox.isChecked = false
                holder.title.state = TaskTitleView.OVERDUE
            }
            else -> {
                //NORMAL
                holder.checkbox.isChecked = false
                holder.title.state = TaskTitleView.NORMAL
            }
        }
    }

    inner class TaskViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val title: TaskTitleView = itemView.findViewById(R.id.item_tv_title)
